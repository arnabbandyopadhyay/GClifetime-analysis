...
void cell::set_north_polarity(space &l) {
   ...
   // north[] contains a normed vector pointint to the upper part of the reaction volume
...
//double frag_cell::frac_average[max_gc] ={ 0.};
...
void frag_cell::set_forbidden_counts(const long &j, double * target, double * old, space &l, hstat &hs) {
   /* OPTION: MOVE_ANALYSIS
    * ++hs.n_move_forbidden;
    ...
    * if (frac_forbid<=0.) ++hs.n_move_forbidden_back;
    */
}
// ============================================================
      ...
      for (int f = 0; f < volume; f++) {
         if (maymoveit[f] == 1) {
            if ((drandom() < p * distance_list[f] / average_distance) || (p == -1)) {
               /* OPTION: MOVE_ANALYSIS
                * ++hs.n_try_move; // remember the number of tries of fragment movement
                */
            } else {
               maymoveit[f] = 0;
            }
            ...
            if (used_nn > 0) {
                  ...
                  if (decomp != j) {
                     ...
                     if (decomp == -1) {
                        ...
                        if (foundpath == 0) {
                           ...
                           /* OPTION: MOVE_ANALYSIS
                            * set_forbidden_counts(j,target_point,oldbary,l,hs);
                            */
               ...
               if (newind != j) {
                  ...
                  while (howmany > 0 && done == 0) {
                     ...
                     if (frag2j != -1) {
                        /* OPTION: MOVE_ANALYSIS
                         * if (maymoveit[frag2j]==1) ++hs.n_move_removed;
                         */
                        maymoveit[frag2j] = 0;
                        done = 1;
                     } else {
                   ...
                   * if (frac_self<=0.) ++hs.n_move_self_back;
                   * }
                   */
               }
               // cout<<"end.\n";
...
double frag_cell::fragmove(states celltype,
   ...
   if (use_D_correction == 1) {
      ...
      fragdiffuse(celltype, tolerance_min, tolerance_steepness, li, p, 1, 0, barycenter, l, hs);
      ...
      if (volume > 1) {
          ...
          * long done_old=hs.n_move_done;
          * long try_old=hs.n_try_move;
          * long forbid_old=hs.n_move_forbidden;
          * long forbid_back_old=hs.n_move_forbidden_back;
          * long removed_old=hs.n_move_removed;
          * long self_old=hs.n_move_self;
          * long self_back_old=hs.n_move_self_back;
          */
          ...
          * hs.n_move_done=done_old;
          * hs.n_try_move=try_old;
          * hs.n_move_forbidden=forbid_old;
          * hs.n_move_forbidden_back=forbid_back_old;
          * hs.n_move_removed=removed_old;
          * hs.n_move_self=self_old;
          * hs.n_move_self_back=self_back_old;
          */
      }
