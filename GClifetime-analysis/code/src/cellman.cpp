...
//int cellman::mutation_frequency[max_mutation_bin][MAX_GCS] = { 0 };
...
cellman::cellman(Parameter &par, space &l, sigs &s, AffinitySpace &shape, ofstream &ana, hstat &hs)
   ...
   if (do_inject_BrdU) {
     ...
     brdu_cell_t.open(hyphasma2::prefix_files("brdu_t.out"));
     brdu_cell_t << "! Time since injection : Time : Generation : BrdU bin (centre) "
		 << ": total # of cells\n";
     brdu_cell_t.close();
     ofstream brduphases;
     brduphases.open(hyphasma2::prefix_files("cellcycle_phases_brdu.out"), ofstream::app);
   ...
   for (int n = 0; n < hstat::max_mutation_bin; n++) {
      hs.mutation_frequency[n] = 0;
   }
    ...
    */
   set_pars(par, l, hs);   // sets parameters dependent on time (can be repeated)
   ...
   while (n_founder < par.Value.totalB) {
	...
	seederCB.attribute_DEC205(p_DEC205);
      ...
      if (put_CB(getpos, seederCB, l, shape) != -1) {
         ...
         hs.cb_obj.average_seeder_affinity
            = (n_founder * hs.cb_obj.average_seeder_affinity
   ...
   if ((par.Value.use_ab_dynamics > 0) && (par.Value.initial_ab_affinity < 0)) {
      ...
      // #### this is wrong because max_affinity shoud be set to the best affinity within the seeder
      // cells
   ...
   */
   ...
   movie.open(hyphasma2::prefix_files("movie"));
   ...
   if (show_mode != islet) {
      ...
      // mkgifs.open(hyphasma2::prefix_files("mkgifs"));
      // mkgifs<<"ppmtogif xy_ini.ppm > xy_ini.gif\n";

      dark_zone.open(hyphasma2::prefix_files("zone_da.out"));
      ...
      dark_zone << "! t #CB/#all #notrecycledCB/#all #CC/#all #CB/#CC #nrCB/#CC "
                << "#all #CB #CC #nrCB\n";
      light_zone.open(hyphasma2::prefix_files("zone_li.out"));
      ...
      xsums[CB].open(hyphasma2::prefix_files("xsumcb.out"));
	 ...
	 << " frac notempty-FDC : frac notempty-FDC-out : CB+CC+out : BC integral\n";
      ...
      ofstream cycle_phases(hyphasma2::prefix_files("cellcycle_phases.out"));
      cycle_phases << "! time[hr] number of CBs (+selected CCs in delay mode)"
		   << "[normal,differentiate,G1,G0,S,G2,M,divide,stop_dividing]\n";
      cycle_phases.close();
      ofstream cycle_phases_zones(hyphasma2::prefix_files("cellcycle_phases_zones.out"));
      ...
      ofstream antigens_out(hyphasma2::prefix_files("antigens.out"));
      antigens_out
         << "! Free antigen presented on FDCs for all antigen types separately (columns)"
	 << " [# of ag_portions] \n";
      antigens_out.close();

      ofstream ics_out(hyphasma2::prefix_files("ics.out"));
      ics_out << "! Immune complexes on FDCs for all antigen types separately (columns) [Mol] \n";
      ics_out.close();

      ofstream icsfrac_out(hyphasma2::prefix_files("ics_frac.out"));
      icsfrac_out << "! Fraction of immune complex versus total antigen"
		  <<" for each antigen type separately (columns)\n";
      icsfrac_out.close();

      cum_ag.open(hyphasma2::prefix_files("cum_ag.txt"), std::ios_base::app);

      xtc.open(hyphasma2::prefix_files("tc.out"));
      xtc << "! t  .  #tc  .  #tc0bc  .  #tc1bc  .  #tc2bc  ....  #tc6bc\n";

      xdec205.open(hyphasma2::prefix_files("dec205.out"));
      ...
      aff_cb_lz.open(hyphasma2::prefix_files("aff_cb_lz.out"));
      aff_cb_lz << "! t . LZ-CB-bin0 . LZ-CB-DEC+-bin0 . ... \n";
      aff_cc_lz.open(hyphasma2::prefix_files("aff_cc_lz.out"));
      aff_cc_lz << "! t . LZ-CC-bin0 . LZ-CC-DEC+-bin0 . ... \n";
      aff_out_lz.open(hyphasma2::prefix_files("aff_out_lz.out"));
      aff_out_lz << "! t . LZ-OUT-bin0 ... \n";
      aff_cb_dz.open(hyphasma2::prefix_files("aff_cb_dz.out"));
      aff_cb_dz << "! t . DZ-CB-bin0 . DZ-CB-DEC+-bin0 . ... \n";
      aff_cc_dz.open(hyphasma2::prefix_files("aff_cc_dz.out"));
      aff_cc_dz << "! t . DZ-CC-bin0 . DZ-CC-DEC+-bin0 . ... \n";
      aff_out_dz.open(hyphasma2::prefix_files("aff_out_dz.out"));
      aff_out_dz << "! t . DZ-OUT-bin0 ... \n";
   }

   if ((show_mode != islet) && (outputfiles == 0)) {
      // Write for movement analysis only
      movements.open(hyphasma2::prefix_files("movement.out"));
      ...
      move_vdt.open(hyphasma2::prefix_files("move_vdt.out"));
      ...
      axis.open(hyphasma2::prefix_files("axis.out"));
      ...
      polarities.open(hyphasma2::prefix_files("polarities.out"));
      polarities << "! times of polarity changes\n"
		 << "! cell#   time[min]   0\n";
      corrs.open(hyphasma2::prefix_files("corrs.out"));
   }
   //
   if (outputfiles == 0) {
      prolog.open(hyphasma2::prefix_files("prolog.out"));
      prolog << "! time  #prol_nn  #prol_diagn  #prol_othern  failures  distance\n";
      /*
       * cbquality.open(hyphasma2::prefix_files("cbqual.out"));
   ...
   //
   if (show_mode != islet) {
     cbhighaff.open(hyphasma2::prefix_files("cbha.out"));
     cbhighaff << "! Quality of all, non-recycled, recycled CBs in %\n";
     cbhighaff << "time   all   non-recycled   recycled\n";
     
     mutation_out.open(hyphasma2::prefix_files("mutation.out"));
     ...
     ag_collected.open(hyphasma2::prefix_files("ag_collected.out"));
     ag_collected
     ...
     ndivtime.open(hyphasma2::prefix_files("ndivtime.out"));
     ndivtime << "! Number of divisions attributed to selected BCs: time.average.sd\n";
     mutation_time.open(hyphasma2::prefix_files("mutation_time.out"));
   ...
   if ((show_mode == islet) && (cellbeta::LOCAL_FILES == false)) {
      ...
      beta_a.open(hyphasma2::prefix_files("beta_a.out"));
      beta_b.open(hyphasma2::prefix_files("beta_b.out"));
      beta_i.open(hyphasma2::prefix_files("beta_i.out"));
      beta_r.open(hyphasma2::prefix_files("beta_r.out"));
      beta_n.open(hyphasma2::prefix_files("beta_n.out"));
      beta_g.open(hyphasma2::prefix_files("beta_g.out"));
   }
...
double cellman::return_freeag(int agno){
    double freeag=0;
    for(int f=0;f<FDC_list.benutzt();f++){
        freeag += FDC_list[f].get_total_antigen(agno);
    }
    return freeag;
}

int cellman::count_dz(space &l){
     int count;
     long list;

     for(int i=l.zone_separator;i<l.pointnumber;i++){
        list=l.cellknot[i].listi;

        if((l.cellknot[i].cell==CB)||(l.cellknot[i].cell==CC && CC_list[list].state!=apoptosis)){
            count=count+1;
        }
     }
...
int cellman::count_lz(space &l){
     int count;

     long list;

     for(int i=0;i<l.zone_separator;i++){
        list=l.cellknot[i].listi;
...
void cellman::write_cellaff_allAGS(int Hno, double time, space &l, SS &newshape) {
    ...
    for(long n=0;n<l.pointnumber;n++){
      ...
      if(l.cellknot[n].cell==out){
            ...
            long li=l.cellknot[n].listi;
            
            // write position,ncells,aff to all ags
            
            OUTaff_GC << time << " " << n << " "<< 1 << " ";
            
            for(int j=0;j<newshape.get_n_Antigen();j++){
            
                long agpos=newshape.get_Antigen(j);    
                OUTaff_GC << newshape.affinity_norm(OUT_list[li].pos_ss, agpos) << " ";
      ...
      if(l.cellknot[n].cell==CB){
            ...
            for(int j=0;j<newshape.get_n_Antigen();j++){
                ...
                long agpos=newshape.get_Antigen(j);    
                CBaff_GC << newshape.affinity_norm(CB_list[li].pos_ss, agpos) << " ";
      ...
      if(l.cellknot[n].cell==CC){
            ...
            for(int j=0;j<newshape.get_n_Antigen();j++){
                ...
                long agpos=newshape.get_Antigen(j);    
                CCaff_GC << newshape.affinity_norm(CC_list[li].pos_ss, agpos) << " ";
...
double cellman::gcvol(){

    double gcvol,CBn,CCn;
...
void cellman::attribute_DEC205() {
   ...
   for (long i = 0; i < noCB; ++i) {
	 ...
	 //if (drandom(1.) < cellCB::p_pMHCdeficient) { CB_list[i].pMHCdeficient = true; }
	 // +++++++++++++++ OPTION pMHCdeficient ++++++++++++++++++++++++++++++++
         ++nCB;
      }
   }
   ...
   for (long i = 0; i < noCC; ++i) {
      ...
      if (drandom(1.) < p_DEC205) {
         ...
         // +++++++++++++++ OPTION pMHCdeficient ++++++++++++++++++++++++++++++++
	 //if (drandom(1.) < cellCB::p_pMHCdeficient) { CC_list[i].pMHCdeficient = true; }
	 // +++++++++++++++ OPTION pMHCdeficient ++++++++++++++++++++++++++++++++
         ++nCC;
      }
   }
...
long cellman::try2exchange_cells(long &i,
   ...
   if ((nn_index != -1)
       && ((nn_type == CB) || (nn_type == CC) || (nn_type == TC) || (nn_type == out)
           || (nn_type == BETA))) {
...
short int cellman::differ2CC_CB(long i, long li, space &l, AffinitySpace &shape, hstat &hs) {
   ...
   if (inject_antiDEC205OVA) {
     ...
     // Thus, those who came from a TB interaction now get inactivated if not within the time window
   }
   // +++++DEC+++++
...
short int cellman::CB_differ2OUT(long i, long li, space &l, AffinitySpace &shape, hstat &hs) {
   ...
   if (CB_list[li].n_mutation < hstat::max_mutation_bin) {
      ++hs.mutation_frequency[CB_list[li].n_mutation];
   } else {
      ++hs.mutation_frequency[hstat::max_mutation_bin - 1];
   }
   ...
   shape.correct_average_affinity(sout, newOUT.pos_ss, hs.out_obj.average_affinity);
   if (aff > hs.out_obj.max_affinity) {
      hs.out_obj.max_affinity = aff;
   }
...
short cellman::BCinflux(space &l, AffinitySpace &shape, sigs &s, hstat &hs) {
   if (p_BCinflux == 0) {
   ...
   if (drandom() < p_in) {
    ...
    newCB.make_CB_new(hs);    // sets the state of the cell to CB and resets its internal clock
...
short cellman::proliferate_CB(long i, long li, space &l, AffinitySpace &shape, hstat &hs) {
   short err = 1;
   long newli;
   ...
   if (cellCB::target_volume == 1) {
	 ...
	 // <<newCB.state<<" n_divisions2do="<<newCB.n_divisions2do<<",";
         ...
         newCB.make_CB_new(hs);     // newCB.state=cb_normal;
         CB_list[li].make_CB_new(hs);
   ...
   if (err == 0) {
      ...
      if (retain_ag) {
         ...
         // By standard both daughters have the full amount of nFDCcontacts
...
void cellman::calc_CB(long * m,
   ...
   for (n = 0; n < mlang; n++) {
      ...
      CB_list[li].get_new_state(m[n], dt, l, s, hs);
      // if (CB_list[li].state>cb_G2)
      ...
      if (CB_list[li].status == necrotic) {
         ...
         if (cellCB::fixed_number_of_divisions()) {
            ...
            if (CB_list[li].state == cb_divide) {
               ...
               errp = proliferate_CB(m[n], li, l, shape, hs);
               // cerr<<"errp="<<errp<<" s="<<CB_list[li].state<<"
               // a="<<CB_list[li].time_of_cycle_state_switch<<";;; ";
            } else if (CB_list[li].state == cb_stop_dividing) {
               errd = CB_list[li].ask_differentiate(hs);       // errd==0 if, yes, differentiate!
               if (errd == 0) {
                  ...
                  if (CB_list[li].diff2output) {
                     CB_differ2OUT(m[n], li, l, shape, hs);
                  } else {
                     differ2CC_CB(m[n], li, l, shape, hs);
                  }
               }
            }
         } else {
            ...
            if ((chose == 1) && (errp != 0)) {
               errd = CB_list[li].ask_differentiate(hs);
               if (errd == 0) {
                  if (CB_list[li].diff2output) {
                     CB_differ2OUT(m[n], li, l, shape, hs);
                  } else {
         ...
         if ((errd != 0) && (CB_list[li].state != cb_M)) {
            // call movement:
            double moved = CB_list[li].move(li, l, s, trackdata, time, hs);
   ...
   BCinflux(l, shape, s, hs);
}
// =======================================================
// Centrocytes ===========================================
// =======================================================

short int cellman::put_CC(long i, cellCC &newCC, space &l, AffinitySpace &shape, hstat &hs) {
   ...
   CC_list[li].trackfate_initialize(hs);
...
short int cellman::del_CC(long i, long li, space &l, hstat &hs) {
  CC_list[li].trackfate(time,true);
  CC_list[li].writetrackfate(hs);
  // remove on lattice
  l.clear_knot(i);
  // l.knot[i].cell=empty;
  // l.knot[i].listi=-1;
...
short cellman::CC_differ2out(long i, long li, space &l, AffinitySpace &shape, hstat &hs) {
   // adjust global counters
   ++n_outs;
   ...
   if (CC_list[li].n_mutation < hstat::max_mutation_bin) {
      ++hs.mutation_frequency[CC_list[li].n_mutation];
   } else {
      ++hs.mutation_frequency[hstat::max_mutation_bin - 1];
   }
...
short cellman::CC_differ2CB(long i, long li, space &l, AffinitySpace &shape, hstat &hs) {
   ++n_recycling_events;
   // alte Werte in neuen CB uebernehmen
   cellCB newCB;
   // Werte vom CC uebernehmen (volume, fragment ... definieren)
   newCB = CC_list[li];
   newCB.set_CBfromCC(newCB, CC_list[li], hs);// settings from operator
   ...
   ++newCB.n_recycling;
   // if ag is not retained, reset this variable (by standard it is transmitted from nFDCcontacts)
   if (retain_ag == false) {
      newCB.retained_ag = 0.;
   ...
   newCB.set_mutation_after_TC(shape, hs);
   ...
   if (((CC_list[li].DEC205_ova == false) && (CC_list[li].dif_delay > 0.))
      ...
      newCB.transmit_CCdelay2cycle(CC_list[li].selected_clock, hs);
   }
...
short int cellman::get_into_FDC_contact(long li, AffinitySpace &shape, AntibodyDyn &Ab,
                    space &l, TRACK &td, double &time, hstat &hs) {
  short int err = 1;
  long fdc_i;
  ...
  if (fdc_i != -1) {
    ...
    threshold = hs.out_obj.average_affinity;
      } else if (cellOUT::use_threshold == 2) {
    threshold = hs.out_obj.max_affinity;
...
void cellman::calc_CC(long * m,
  ...
  cellCC::set_differentiation(time, hs);   // to update time-dependent probabilities



  for (n = 0; n < mlang; n++) {
    // Get index on the cell list
     // if(time>647) cerr<<"value of n"<<n;
    ...
    //if(time>647) cerr<<"state of n"<<CC_list[li].state;
	...
	if (not (cellCC::collectFDCsignals)) {
      ...
      err = CC_list[li].apoptose(time, shape, hs);
	...
	if (err == 1) {
      ...
      err = CC_list[li].stop_collecting_FDCsignals(shape, time, dt, hs);
	  ...
	  if (err == 0) { // continue collecting FDC signals:
        err = get_into_FDC_contact(li, shape, Ab, l, trackdata, time, hs);
    ...
    case contact: { // this means contact to FDC.
      // cout<<"contact ... ";
      CC_list[li].selected_clock += dt;
      // Assumption: No apoptosis if in contact to FDC.
      // The following controls the duration of selection process at FDC
      err = CC_list[li].select(shape, hs);
    ...
    if (CC_list[li].apoptose(time, shape, hs) == 0) {
	  ...
	  if (err > 0) {
	    ...
	    if (CC_list[li].try2findTFH()) { 
	      ...
	      /*
	      cout << "TCtest1[" << li << "]: tc_i=" << tc_i
		   << "; CXCL13responsive=" << CC_list[li].responsive2signal[CXCL13] 
		   << "\n";
	      */
	    }
	    if (tc_i == -1 || CC_list[li].same_TC_as_before(tc_i) ) {
	      // Failure to find Tfh:
		...
		*/
	      ...
	      if (err > 0) { CC_list[li].move(li, l, s, trackdata, time); }
	    } else { 
          CC_list[li].bind_TC(time, TC_list[l.cellknot[tc_i].listi], l, shape, hs);
	...
	if (CC_list[li].final_differentiation()) {
      CC_differ2out(m[n], li, l, shape, hs);
	  CCexists = false;
	} else {
	  // nomove=
	  CC_list[li].move(li, l, s, trackdata, time);
    ...
    err = CC_list[li].dif2OUTorCB(dt, hs);
	if (err == 0) {
	  // nomove=
	  CC_list[li].move(li, l, s, trackdata, time);
	} else {
	  if (err == 2) {
        ...
        CC_differ2CB(m[n], li, l, shape, hs);
	    CCexists = false;
	  } else {
	    if (err == 1) {
    ...
    case apoptosis: {
      ...
      if (CC_list[li].macrophagocyte(shape, hs) == 1) {
...
void cellman::calc_FDC(space &l, sigs &s, AntibodyDyn &Ab, AffinitySpace &AS, hstat &hs) {
   long n;
   // Randomize the sequence of points that are actualized
   ...
   for (n = 0; n < max; n++) {
      ...
      FDC_list[l.cellknot[m[n]].FDClisti].signal_production(l.knot[m[n]].guest_grid_pointer, s, hs);
      // rate equation for production of immune complex:
      if (s.signal_use[antibody] == 1) {
         if (AS.get_n_Antigen() == 1) {
            FDC_list[l.cellknot[m[n]].FDClisti].mk_immune_complex(dt, s, hs);
         } else {
         ...
         FDC_list[l.cellknot[m[n]].FDClisti].mk_immune_complex(dt, Ab, AS, hs);
      }
...
void cellman::calc_OUT(space &l, sigs &s, AffinitySpace &shape, dynarray<long> &redo) {
   ...
   for (n = 0; n < max; n++) {
      ...
      // Output cells which are at the border of the system leave it, others diffuse
      ...
      // if (eject_out(m[n],li,l,shape)!=0) OUT_list[li].diffuse(li,l);
   }
   // cout<<"end calc_OUT.\n";
}
// =======================================================
...
void cellman::calc_BETA(long * m, long mlang, space &l, sigs &s, dynarray<long> &redo, hstat &hs) {
   ...
   for (n = 0; n < mlang; n++) {
      ...
      if (BETA_list[li].status == necrotic) {
         ...
         // call movement:
         BETA_list[li].move(li, l, s, trackdata, time, hs);
...
void cellman::zone_files(suffix tnr, space &l) {
   ...
   // Stream oeffnen
   ofstream ff(hyphasma2::prefix_files(datname));
...
void cellman::show_foxo(double time, long* nCC) {
  ...
  ofstream foxofile;
  foxofile.open(hyphasma2::prefix_files("foxoCC.out"), ofstream::app);
  foxofile << time << "  "
	   << time/24. << "   ";
...
void cellman::showFDCfailure(double& t) {
  ofstream failure;
  failure.open(hyphasma2::prefix_files("FDCagfailure.out"), ofstream::app);
  failure << t << "  " << t/24. << "    "
	  << TestedFDCsiteVoidOfAg << "  ";
  double ratio = 0.;
...
void cellman::show_cycle_phases(long zone_separator) {
  ...
  ofstream cycle_phases(hyphasma2::prefix_files("cellcycle_phases.out"), ofstream::app);
  cycle_phases << time << "    "
	       << totalBC << "    ";
   ...
   */
  ofstream cycle_phases_zones(hyphasma2::prefix_files("cellcycle_phases_zones.out"), ofstream::app);
...
double cellman::count_out(space &l){
...
double cellman::calc_outaff_sum(int agno, int nogc, space &l, SS &newshape){

double outaff_sum=0.0;
long agpos=newshape.get_Antigen(agno);

for(long n=0;n<l.pointnumber;n++){
    if(l.cellknot[n].cell==out){
      long li=l.cellknot[n].listi;
      outaff_sum=outaff_sum+(newshape.affinity_norm(OUT_list[li].pos_ss,agpos));
    }

}

outaff_sum=outaff_sum*nogc;
...
void cellman::mk_cell_sum(space &l, sigs &s, AntibodyDyn &Ab, AffinitySpace &shape) {
  ...
  for (int i = 0; i < nIg_classes; i++) {
   ...
   xvolume << time/24 << "   " << notempty << "   " << notempty - FDCpointnumber << "   " << nd_CB
      + nd_CC[apoptosis + 1]
   ...
   for (long n = 0; n < l.pointnumber; n++) {
      ...
      if ((l.knot[n].status != external) && (l.cellknot[n].cell == TC)) {
         // At first add ths to the number of TC
         ++ntc;
   ...
   if (cellFDC::use_antigen == 1) {
	 ...
	 ics += FDC_list[f].get_total_immune_complex(AntibodyDyn::antibodies_resolution);
	 // and for each antigen separately:
	 for (int a = 0; a < n_ags; a++) {
	   ics_ags[a] += 
	     FDC_list[f].get_total_immune_complex(a,AntibodyDyn::antibodies_resolution);
	 }
       } else {
	 // ###multiAg: does multiple Ags work without bins?
     ...
     ofstream relIP;
     gcn.open(hyphasma2::prefix_files("GC_new.out"),ofstream::app);
     IP.open(hyphasma2::prefix_files("IP.out"),ofstream::app);
     self_ic.open(hyphasma2::prefix_files("self_ic.out"),ofstream::app);
     self_ab.open(hyphasma2::prefix_files("self_ab.out"),ofstream::app);
     self_ic_pergc.open(hyphasma2::prefix_files("self_ic_pergc.out"),ofstream::app);
     self_ab_pergc.open(hyphasma2::prefix_files("self_ab_pergc.out"),ofstream::app);
     relIP.open(hyphasma2::prefix_files("relIP.out"),ofstream::app);


     IP << time/24 << "    " << (ics / cellFDC::ag_threshold)/ (free_ag+(ics / cellFDC::ag_threshold)) << "\n";
     self_ic << time/24<<" ";
     self_ic_pergc << time/24 << " ";
     self_ab << time/24 << " ";
     self_ab_pergc << time/24 << " ";

     for(int i=0;i<int(Ab.ab_bins.size());i++){
     double ImmunePower=0;
     double ImmunePower_pergc=0;

     ImmunePower=Ab.ab_bins[i].get_total_self_ic();
     ImmunePower_pergc=Ab.ab_bins[i].get_total_self_ic_pergc();

     self_ic << ImmunePower/(AntibodyBin::gfree)<< " ";
     self_ic_pergc << ImmunePower_pergc/(AntibodyBin::gfree_pergc) << " ";


     double self_ab_tt=0;
     double self_ab_tt_pergc=0;
     self_ab_tt=Ab.ab_bins[i].get_total_self_ab();
     self_ab_tt_pergc=Ab.ab_bins[i].get_total_self_ab_pergc();

     self_ab << self_ab_tt << " ";
     self_ab_pergc << self_ab_tt_pergc << " ";
     }
     self_ic << endl;
     self_ic_pergc << endl;
     self_ab << endl;
     ...
     if(time< 504.0+0.5*dt &&time>504-0.5*dt){
     gcn << " " << (ics / cellFDC::ag_threshold)/ (free_ag+(ics / cellFDC::ag_threshold));
     }
     gcn.close();
     //self_ic.close();


     ag_presentation << time << "   " << free_ag << "   " << ics / cellFDC::ag_threshold << "   " << total_ab / cellFDC::ag_threshold << "   " << free_ag* cellFDC::ag_threshold << "   " << ics << "   " << total_ab<<"   "<<ics*cellFDC::ag_threshold <<"   "<<total_ab*cellFDC::ag_threshold<< "\n";
     // now write out ag and ic for each antigen separately as well as the fraction of both:
     ofstream multiag_file(hyphasma2::prefix_files("antigens.out"), ofstream::app);
     for (int a = 0; a < n_ags; a++) {
       multiag_file << free_ags[a] << "   ";
     }
     multiag_file << "\n";
     multiag_file.close();
     ofstream multiic_file(hyphasma2::prefix_files("ics.out"), ofstream::app);
     for (int a = 0; a < n_ags; a++) {
       multiic_file << ics_ags[a] << "   ";
     }
     multiic_file << "\n";
     multiic_file.close();
     ofstream multiicfrag_file(hyphasma2::prefix_files("ics_frac.out"), ofstream::app);
     for (int a = 0; a < n_ags; a++) {
       multiicfrag_file << time/24 << " " << ics_ags[a] / (ics_ags[a] + free_ags[a] * cellFDC::ag_threshold)
			<< "   ";
     }
...
void cellman::xfiles(suffix tnr, space &l) {
   ...
   ofstream ffdx;
   if (opendx) {
      ffdx.open(hyphasma2::prefix_files(opendxname));
      ffdx.setf(ios::floatfield);
   }
   ofstream ffpov(hyphasma2::prefix_files(povname));
...
void cellman::show_BrdU() {
   ...
   ofstream brdu_out;
   brdu_out.open(hyphasma2::prefix_files("brdu.out"));
...
void cellman::show_cell_in_BrdU() {
   ...
   brdu_cell_out.open(hyphasma2::prefix_files("brdu_cell.out"));
   brdu_cell_out
...
void cellman::show_BrdU_at_time() {
  /* Goes through all BC and attributes the cellular BrdU level to BrdU bins */
   ...
   if (tt < 0) { tt = 0; }
   ofstream brdu_cell_t;
   brdu_cell_t.open(hyphasma2::prefix_files("brdu_t.out"), ofstream::app);
   for (int j = 0; j < BrdU_bins; ++j) {
     brdu_cell_t << tt << "  " << time << "   " 
		 << j << "    " << BrdU_level[j]
		 << "    " << BrdU_array[j] << "\n";
...
void cellman::show_ag_loaded() {
   ...
   ofstream ag_out;
   ag_out.open(hyphasma2::prefix_files("ag_loaded.out"));
...
void cellman::show_ag_BrdU(bool ini) {
   ofstream ag_brdu;
   if (ini) {
      ag_brdu.open(hyphasma2::prefix_files("ag_brdu_ini.out"));
   } else {
      ag_brdu.open(hyphasma2::prefix_files("ag_brdu.out"));
   }
   ag_brdu << "! brdu : ag\n";
   for (long i = 0; i < CB_list.benutzt(); ++i) {
...
void cellman::write_mutations(double time) {
  //cerr << "Start write_mutations(double) ... \n";
   ...
   mutation_histo.open(hyphasma2::prefix_files("mutation_histo.out"), ofstream::app);
   for (int n = 0; n <= max_mutation; n++) {
...
void cellman::write_Ig_class_output() {
  ofstream igoutput;
  igoutput.open(hyphasma2::prefix_files("xsumout_ig.out"), ofstream::app);
...
void cellman::write_final(const double &deltax, hstat &hs) {
   // write into velocity files:

   //  if (outputfiles==0) {
   double average_v = 0;
   ofstream vmean;
   vmean.open(hyphasma2::prefix_files("vmean.out"));
   ...
   long all_moves = 0;
   ofstream vdist;
   vdist.open(hyphasma2::prefix_files("vdist.out"));
   ...
   vdist.open(hyphasma2::prefix_files("vdistd.out"));
   vdist.setf(ios::scientific, ios::floatfield);
   ...
   ofstream encount;
   encount.open(hyphasma2::prefix_files("fdcencount.out"));
   ...
   for (int a = 0; a < hstat::fdc_max_encounters; a++) {
      encount << a << "   " << hs.cc_obj.fdc_encounters[a] << "\n";
   }
   encount.close();

   ofstream muthisto(hyphasma2::prefix_files("mutation_out_histo.out"));
   for (int i = 0; i < hstat::max_mutation_bin; i++) {
      muthisto << i << "   " << hs.mutation_frequency[i] << endl;
   }
   muthisto.close();

   cellCB::show_cummulative_number_of_divisions(hs);
   cellCC::show_cummulative_ag_collection(hs);
   cellCB::show_cummulative_mutation_prob(hs);
   cellCB::show_cell_cycle_phase_duration(hs);
   ...
   while (fff.eof() == 0) {
      ...
      if (dati == ind) {
         if (target_open == false) {
            ...
            strcat(foutput, ".out");
            target.open(hyphasma2::prefix_files(foutput));
            target_open = true;
         }
         for (int j = 1; j < columns; j++) {
            target << dat[j] << "  ";
...
void cellman::write_cell_specific(hstat &hs) {
  cellCC::write_histograms(hs);
}
// =======================================================
// Main routines =========================================
// =======================================================

void cellman::set_pars(Parameter &par, space &l, hstat &hs) {
   cellCB::set_statics(time, par, hs);
   cellCC::set_statics(time, par, hs);
   cellFDC::set_statics(time, par, hs);
}
...
void cellman::time_step(short int ss_save, short int day_save,
                        ...
                        AntibodyDyn &Ab,
                        AffinitySpace &shape,
                        ofstream &ana, Parameter &p, hstat &hs) {
    //cerr<<"Start timestep ... (time="<<time<<")\n";

    //if(time>647) cerr<<"start timestep";
 ...
 /*
   ...
   if (inject_antiDEC205OVA && time < inject_antiDEC205OVA_t0) {
     for (int k=0; k <= hstat::max_n_of_divisions; k++)
       { hs.cb_obj.cummulative_attributed_n_of_divisions[k] = 0; }
   }
   ...
   if(hyphasma2::PCsUndergoApoptosis==true){
       shape.PM_apoptosis(soutextproduce,dt);
   }
   // ##### note that this might be done always -- not only for usage of antibody bins.
   if (use_antibody_bins)
     {
       Ab.produce_antibodies_outside(shape, hs);
   }   // Philippe : I don't understand how antibodies are produces in the other case (not bins)


   //Ab.ab_bins[0].attributeABfromAS(shape);
   //Ab.ab_bins[0].write_self_ab(time);

   if(p.Value.antibodies_production>0 && p.Value.antibodies_resolution>0)
   {
       for(int i=0;i<int(Ab.ab_bins.size());i++){
           Ab.ab_bins[i].clear_self_ics();
           Ab.ab_bins[i].make_self_ic(p);
       }
   }
   if(delay_gc >= 0)
   {

   // Ab.ab_bins[0].update_ab_new(ab_ext, ntcount);

    }


   //Ab.ab_bins[0].write_ab_tc(time);


   //cerr<<"vor FDC ... \n";
   calc_FDC(l, s, Ab, shape, hs);
   ...
   calc_CC(mCC, mCClang, l, s, Ab, shape, redo, hs);

   // cerr<<"vor CB ... "<<mCBlang<<"\n";
   calc_CB(mCB, mCBlang, ss_save, l, s, shape, redo, hs);
   // cerr<<"nach CB. \n";
   ...
   calc_BETA(mBETA, mBETAlang, l, s, redo, hs);
   // cerr<<"nach BETA. \n";
   ...
   if ((show_mode != islet) && (ss_save == 1)) {
	...
	show_BCstate_in_BrdUpositive();
      ...
      cellCB::show_number_of_divisions(time, ndivtime, hs);

      double muta_av = 0, muta_sd = 0;
      get_average_mutation_prob(muta_av, muta_sd);
     cellCB::show_mutation_prob(time, muta_av, muta_sd, mutation_time, hs);
   }
   if ((show_mode != islet) && (day_save == 1)) {


     cellCC::update_histograms(hs);
