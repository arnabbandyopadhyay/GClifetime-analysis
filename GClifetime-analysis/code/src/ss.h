...
class SS: public AffinitySpace {
  public:
   ...
   double Cellaff_sum(int agindex,int nogc,cells celltype);
   double nCells(cells celltype);

   // (thein) for multiple GC version: readouts of PCaff towards all ags for every GC simulated
   void write_PCaff_allAGS(int Hno);
   void write_cellaff_allAGS(int Hno,double time);
   ...
   void PM_apoptosis(cells a, double dt);
   ...
   void Call_getStatistics(double &_OUT_haffinity,
                      double &_OUT_steepness,
                      double &_CB_haffinity,
                      double &_CC_haffinity) {
      getStatistics(_OUT_haffinity, _OUT_steepness, _CB_haffinity, _CC_haffinity);
   }
  ...
  private:
   ...
   void calc_nplasma(double * n_plasma, int ag_index);
   ...
   long int get_random_SS_position_ag(double mindist, double maxdist,long agindex);
