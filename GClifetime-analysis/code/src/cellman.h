...
public:
   cellman();
   cellman(Parameter &par, space &l, sigs &s, AffinitySpace &shape, ofstream &ana, hstat &hs);
   ~cellman();


    double delay_gc;
    double return_freeag(int agno);
    int count_dz(space &l);
    int count_lz(space &l);
    double count_out(space &l); //thein
    double calc_outaff_sum(int agno, int nogc, space &l, SS &newshape);
    void write_cellaff_allAGS(int Hno,double time, space &l, SS &newshape);
   ...
   void write_cell_specific(hstat &hs);
   void set_pars(Parameter &par, space &l, hstat &hs);
   ...
   void write_final(const double &deltax, hstat &hs);
   ofstream movie;
   void show_BrdU();
...
//private:
   ...
   short int differ2CC_CB(long i, long li, space &l, AffinitySpace &shape, hstat &hs);
   short int CB_differ2OUT(long i, long li, space &l, AffinitySpace &shape, hstat &hs);
   // cell division
   short proliferate_CB(long i, long li, space &l, AffinitySpace &shape, hstat &hs);
   ...
   short BCinflux(space &l, AffinitySpace &shape, sigs &s, hstat &hs);

   // do everything:
   void calc_CB(long * m,long mlang, short int ss_save,
                space &l, sigs &s, AffinitySpace &shape, dynarray<long> &redo, hstat &hs);
   ...
   short int put_CC(long i, cellCC &newCC, space &l, AffinitySpace &shape, hstat &hs);
   // long int find_CC(long i);
   short int del_CC(long i, long li, space &l, hstat &hs);
   ...
   short CC_differ2CB(long i, long li, space &l, AffinitySpace &shape, hstat &hs);
   short CC_differ2out(long i, long li, space &l, AffinitySpace &shape, hstat &hs);
   ...
   void calc_FDC(space &l, sigs &s, AntibodyDyn &Ab, AffinitySpace &AS, hstat &hs);
   ...
   void calc_BETA(long * m, long mlang, space &l, sigs &s, dynarray<long> &redo, hstat &hs);
   ...
   //static int mutation_frequency[max_mutation_bin][MAX_GCS]; // write this to file in write_final(..)
   ofstream cum_vol;
