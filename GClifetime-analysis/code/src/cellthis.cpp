...
//double cellCB::average_seeder_affinity[MAX_GCS] = {0.0};
...
/*int cellCB::dtphase_frequency[phase_number][dtphase_resolution][MAX_GCS] = {
   { 0 }
};*/
...
//long cellCB::attributed_n_of_divisions[max_n_of_divisions + 1][MAX_GCS] = { };
//long cellCB::cummulative_attributed_n_of_divisions[max_n_of_divisions + 1][MAX_GCS] = { };
//long cellCB::attributed_mutation_prob[mutation_bins][MAX_GCS] = { };
//long cellCB::cummulative_attributed_mutation_prob[mutation_bins][MAX_GCS] = { };

const double cellCB::p_pMHCdeficient = 0.8;
...
cellCB&cellCB::operator =(const cellCC &x) {
   ...
   pMHCdeficient = x.pMHCdeficient;
   IgX.set_class(x.IgX);

   /*double ndivtmp = total_n_of_divisions;
   if (x.pMHC_dependent_number_of_divisions >= 0) {
      ndivtmp = x.pMHC_dependent_number_of_divisions;
      /* This includes the case of DEC205_ova, where pMHC_dependent_number_of_divisions was 
       * already attributed to the CC at the time of selection, when DND was calculated.
   ...
   if (ndivtmpi > hstat::max_n_of_divisions) { ndivtmpi = hstat::max_n_of_divisions; }
   hstat::attributed_n_of_divisions[ndivtmpi][hyphasma2::GCref]++;
   hstat::cummulative_attributed_n_of_divisions[ndivtmpi][hyphasma2::GCref]++;*/
...
void cellCB::set_CBfromCC(cellCB &cb, cellCC &cc, hstat &hs){

    cb.make_CB_new(hs);

    double ndivtmp = total_n_of_divisions;
    if (cc.pMHC_dependent_number_of_divisions >= 0) {
       ndivtmp = cc.pMHC_dependent_number_of_divisions;
       /* This includes the case of DEC205_ova, where pMHC_dependent_number_of_divisions was
        * already attributed to the CC at the time of selection, when DND was calculated.*/
    ...
    int ndivtmpi = n_divisions2do;
    if (ndivtmpi > hstat::max_n_of_divisions) { ndivtmpi = hstat::max_n_of_divisions; }
    hs.cb_obj.attributed_n_of_divisions[ndivtmpi]++;
    hs.cb_obj.cummulative_attributed_n_of_divisions[ndivtmpi]++;
}


void cellCB::make_CB_new(hstat &hs) {
   ...
   if (fixed_number_of_divisions()) {
      // cout<<"set new CB .. ";
      state = cb_G1;
      time_of_cycle_state_switch = set_cycle_state_duration(state, hs);
   }
   cycle_state_time = 0.;
}
...
void cellCB::set_statics(const Parameter &par, space &l, ofstream &ana, hstat &hs) {
   ...
   } else { ana << "  No delay of CB differentiation for division, thus, rate based division.\n"; }
   ...
   if (fixed_number_of_divisions()) { hs.cb_obj.p_dif= p_dif_target; }
...
void cellCB::set_statics(const double &time, const Parameter &par, hstat &hs) {
   if (fixed_number_of_divisions()) { hs.cb_obj.p_dif = p_dif_target; } else if (SMOOTH_DIFFERENTIATION) {
      set_differentiation(time, hs);
   } else if (time >= par.Value.Start_Differentiation - 1.0e-09) {
      hs.cb_obj.p_dif = p_dif_target;
   ...
   } else { hs.cb_obj.p_dif = 0.; }
}
void cellCB::set_differentiation(const double &time, hstat &hs) {
...
void cellCB::attribute_DEC205(double fracofpos) {
  ...
  */
  if (drandom(1.) < fracofpos) { 
    DEC205 = true;
    // +++++++++++++++ OPTION pMHCdeficient ++++++++++++++++++++++++++++++++
    //if (drandom(1.) < p_pMHCdeficient) { pMHCdeficient = true; }
    // +++++++++++++++ OPTION pMHCdeficient ++++++++++++++++++++++++++++++++
  }
}
// =======================================================================

double cellCB::set_cycle_state_duration(centroblasts &s, hstat &hs) {
   // This follows
   ...
   int dt_index = int (phaselength / (2.0 * thisphase / double (hstat::dtphase_resolution - 1)) + 0.5);
   if ((dt_index < 0) || (dt_index > hstat::dtphase_resolution - 1))
   { cout << "Error in cellCB::set_cycle_state_duration(...) \n"; exit(1); }
   ++hs.cb_obj.dtphase_frequency[s - cb_G1][dt_index];
...
void cellCB::transmit_CCdelay2cycle(double waited_time, hstat &hs) {
...
void cellCB::get_new_state(const long &i, double &dt, space &l, sigs &s, hstat &hs) {
...
void cellCB::set_mutation_after_TC(AffinitySpace &shape, hstat &hs) {
   ...
   int mutbin = int (p_mutation * double (hstat::mutation_bins));
   if (mutbin >= hstat::mutation_bins) { mutbin = hstat::mutation_bins - 1; }
   hs.cb_obj.attributed_mutation_prob[mutbin]++;
   hs.cb_obj.cummulative_attributed_mutation_prob[mutbin]++;
}
...
short cellCB::ask_differentiate(hstat &hs) {
   short int err = 1;
...
double cellCB::move(const long &li, space &l, sigs &s, TRACK &td, double &time, hstat &hs) {
   // call polarisation
   ...
   return fragmove(CB,li,diffusion_tolerance_min,diffusion_tolerance_steepness,
                   elongation,smoothmove,p_tension,K_elongation,l, hs);
   // Note that p_move is not needed in the parameter list, as it is a property of class cell.
}
void cellCB::show_number_of_divisions(double &time, ofstream &ndivtime, hstat &hs) {
   // get average and sd since last writing

   double ndiv_av = 0, ndiv_sd = 0;
   int ndiv_nn = 0;
   for (int i = 0; i <= hstat::max_n_of_divisions; i++) {
      ndiv_av += double (hs.cb_obj.attributed_n_of_divisions[i] * i);
      ndiv_nn += hs.cb_obj.attributed_n_of_divisions[i];
   }
   if (ndiv_nn > 0) { ndiv_av /= double (ndiv_nn); } else { ndiv_av = 0; }
   for (int i = 0; i <= hstat::max_n_of_divisions; i++) {
      for (int j = 0; j < hs.cb_obj.attributed_n_of_divisions[i]; j++) {
   ...
   for (int i = 0; i <= hstat::max_n_of_divisions; i++) {
      hs.cb_obj.attributed_n_of_divisions[i] = 0;
   }
}
void cellCB::show_cummulative_number_of_divisions(hstat &hs) {
   ofstream ndivhisto(hyphasma2::prefix_files("ndivhisto.out"));
   ...
   for (int i = 0; i <= hstat::max_n_of_divisions; i++) {
      ndivhisto << i << "   " << hs.cb_obj.cummulative_attributed_n_of_divisions[i] << "\n";
   }
   ndivhisto.close();
}
void cellCB::show_mutation_prob(double &time,
                                double &muta_av,
                                double &muta_sd,
                                ofstream &mutation_time, hstat &hs) {
   // get average and sd since last writing
   double mut_av = 0, mut_sd = 0;
   int mut_nn = 0;
   for (int i = 0; i < hstat::mutation_bins; i++) {
      mut_av += double (hs.cb_obj.attributed_mutation_prob[i]) * double (double (i) + 0.5)
                / double (hstat::mutation_bins);
      mut_nn += hs.cb_obj.attributed_mutation_prob[i];
   }
   if (mut_nn > 0) { mut_av /= double (mut_nn); } else { mut_av = 0; }
   for (int i = 0; i < hstat::mutation_bins; i++) {
      double muttmp = 0;
      for (int j = 0; j < hs.cb_obj.attributed_mutation_prob[i]; j++) {
         muttmp = (double (i) + 0.5) / double (hstat::mutation_bins);
   ...
   // clear the array for the next time interval
   for (int i = 0; i < hstat::mutation_bins; i++) {
      hs.cb_obj.attributed_mutation_prob[i] = 0;
   }
}
void cellCB::show_cummulative_mutation_prob(hstat &hs) {
   ofstream mutset(hyphasma2::prefix_files("mutation_set.out"));
   ...
   for (int i = 0; i < hstat::mutation_bins; i++) {
      mutset << i << "   "
             << (double (i) + 0.5) / double (hstat::mutation_bins) << "   "
             << hs.cb_obj.cummulative_attributed_mutation_prob[i] << "\n";
   }
   mutset.close();
}
void cellCB::show_cell_cycle_phase_duration(hstat &hs) {
   ...
   for (short p = cb_G1; p <= cb_M; p++) {
      if (p == cb_G1) { ccp.open(hyphasma2::prefix_files("cellcycle_duration_g1.out")); } else if (p == cb_G0) {
         ccp.open(hyphasma2::prefix_files("cellcycle_duration_g0.out"));
      } else if (p == cb_S) {
         ccp.open(hyphasma2::prefix_files("cellcycle_duration_s.out"));
      } else if (p == cb_G2) {
         ccp.open(hyphasma2::prefix_files("cellcycle_duration_g2.out"));
      } else if (p == cb_M) {
         ccp.open(hyphasma2::prefix_files("cellcycle_duration_m.out"));
      }
      for (int i = 0; i < hstat::dtphase_resolution; i++) {
         duration = 2.0 * double (i) * dtphase[p] / double (hstat::dtphase_resolution - 1);
         ccp << duration << "   " << hs.cb_obj.dtphase_frequency[p - cb_G1][i] << endl;
      }
      ccp.close();
   }
}
...
cellCC::cellCC() {
   ...
   BCRexpression = 1.0;
   CXCR5failure = 0;
   pMHC_dependent_number_of_divisions = -1;
   fatetracker_n = 0;
}
...
cellCC&cellCC::operator =(const cellCC &x) {
   ...
   pMHC_dependent_number_of_divisions = x.pMHC_dependent_number_of_divisions;
...
double cellCC::rescue_signal_threshold = 1.0; // defined in constructor
...
//int cellCC::fdc_encounters[fdc_max_encounters][MAX_GCS];
...
double cellCC::pMHC_dependent_P_standard = 2.0;
double cellCC::pMHC_dependent_P_min = 1.0;
double cellCC::pMHC_dependent_P_max = 6.0;
double cellCC::pMHC_dependent_K = 8.0;
double cellCC::TFHsignal_dependent_K = 1.5; 
double cellCC::TFHgradient_dependent_K = 1.5; 
double cellCC::pMHC_dependent_nHill = 1.0;
...
//long cellCC::cummulative_ag_collection_all[max_n_of_ag_portions + 1][MAX_GCS] = { 0 };
//long cellCC::cummulative_ag_collection_selected[max_n_of_ag_portions + 1][MAX_GCS] = { 0 };
short cellCC::present_specific_ag2TC = 0;
short cellCC::outputfiles = 1;
double cellCC::write_trackfate = 0;
//
/*histo_monitor cellCC::monitor_pureFDCsearch[MAX_GCS];
histo_monitor cellCC::monitor_nTCcontacts_selected[MAX_GCS];
histo_monitor cellCC::monitor_nTCcontacts_deleted[MAX_GCS];
histo_monitor cellCC::monitor_BsearchTtime_selected[MAX_GCS];
histo_monitor cellCC::monitor_BsearchTtime_deleted[MAX_GCS];
histo_monitor cellCC::monitor_BinteractTtime[MAX_GCS];
histo_monitor cellCC::monitor_TimeBetweenTbindings[MAX_GCS];
histo_monitor cellCC::monitor_TfhSignalAtSelection[MAX_GCS];
histo_monitor cellCC::monitor_TfhSignalSpeed_selected[MAX_GCS];
histo_monitor cellCC::monitor_TfhSignalSpeed_deleted[MAX_GCS];
histo_monitor cellCC::monitor_TFHsignalAUC[MAX_GCS];*/
//
int cellCC::fatetracker_ndt = 8; // number of timesteps between two write events

void cellCC::set_statics(const Parameter &par, space &l, ofstream &ana, hstat &hs) {
  //
  // For centrocytes
  ...
  if (mode_of_apoptosis == 0) {
   ...
   p_sel = par.Value.selection * par.Value.deltat;
   ana << "  CC selection = " << p_sel << "\n";
   ...
   if (par.Value.TFHsignal_decay > 0) {
     /* The following calculates the factor of reducing the tc_signal per time step */
     TFHsignal_decay = exp(-1.0 * par.Value.deltat * log(2.) / par.Value.TFHsignal_decay);
   ...
   */
   ...
   for (int a = 0; a < hstat::fdc_max_encounters; a++) {
      hs.cc_obj.fdc_encounters[a] = 0;
   }
   ...
   if (pMHC_dependent_division || SIND) {
      ...
      pMHC_dependent_P_standard = par.Value.pMHC_dependent_P_standard;
      pMHC_dependent_P_min = par.Value.pMHC_dependent_P_min;
      pMHC_dependent_P_max = par.Value.pMHC_dependent_P_max;
      pMHC_dependent_nHill = par.Value.pMHC_dependent_nHill;
      pMHC_dependent_K = par.Value.pMHC_dependent_K;
      if (pMHC_dependent_K < 0) {
	 /* Determine the K-value from the condition that 
	  * X0 is the amount of pMHC at which Hill(X0) = P0 (= P_standard),
	  * yielding   K = X0 [\frac{Pmax - Pmin}{P0 - Pmin} - 1]^(1/n)
	  */
         pMHC_dependent_K
            = par.Value.pMHC_dependent_pMHC_of_2divisions
              * pow((((pMHC_dependent_P_max - pMHC_dependent_P_min)
                      / (pMHC_dependent_P_standard - pMHC_dependent_P_min)) - 1.0),
                    (1.0 / pMHC_dependent_nHill));
         ana << "  ... calculated K_pMHC of the Hill-function to " 
	     << pMHC_dependent_K << "\n";
      } else {
	 ...
	 /* Determine the K-value from the condition that 
	  * X0 is the amount of TFHsignal at which Hill(X0) = P0 (= P_standard),
	  * yielding   K = X0 [\frac{Pmax - Pmin}{P0 - Pmin} - 1]^(1/n)
	  ...
	  */
         TFHsignal_dependent_K
            = par.Value.TFHsignal_of_P0divisions
              * pow((((pMHC_dependent_P_max - pMHC_dependent_P_min)
                      / (pMHC_dependent_P_standard - pMHC_dependent_P_min)) - 1.0),
                    (1.0 / pMHC_dependent_nHill));
         ana << "  ... calculated K_signal of the Hill-function to " 
	     << TFHsignal_dependent_K << "\n";
      } else {
	  ...
	  */
         TFHgradient_dependent_K
            = par.Value.TFHgradient_of_P0divisions
              * pow((((pMHC_dependent_P_max - pMHC_dependent_P_min)
                      / (pMHC_dependent_P_standard - pMHC_dependent_P_min)) - 1.0),
                    (1.0 / pMHC_dependent_nHill));
       ...
       * meandiv = pMHC_dependent_P_standard divisions.
       * This is calculated from the condition Hill(pMHC_0) = 2 = meandiv, yielding
       * pMHC_0 = K n_sqrt{(meandiv-P_min)/(P_max-meandiv)} 
       */
      // ++++++++++++++++ OPTION ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      double DEC205_ova_pMHC_factor = 5.0; // factor read-off from Fig S5E in Victora Cell 2010
      double meandiv = pMHC_dependent_P_standard; // A value of 2 is given in Gitlin 2014
      // ++++++++++++ end OPTION ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++	 
      if (meandiv <= pMHC_dependent_P_min || meandiv >= pMHC_dependent_P_max) {
	cerr << "ERROR: meandiv = " << meandiv << " in cellCC::set_statics(...) "
	     << "is required to be in the interval ]" << pMHC_dependent_P_min << ","
	     << pMHC_dependent_P_max << "[. Exit simulation.\n";
	exit(1);
      }
       ...
       */
      pMHC_of_DEC205_ova = DEC205_ova_pMHC_factor * pMHC_dependent_K * 
	 pow( ((meandiv - pMHC_dependent_P_min) / (pMHC_dependent_P_max - meandiv)), 
	      (1.0 / pMHC_dependent_nHill) );
  ...
  hs.monitor_pureFDCsearch.initialize(max_value, 100, par.Value.tmin, par.Value.tmax,
   ...
   */
  max_value = 100; 
  hs.monitor_nTCcontacts_selected.initialize(max_value, 100, par.Value.tmin, par.Value.tmax,
					  "ntc_contacts_selected.out", 
					  "nTCcontacts_selected");
  hs.monitor_nTCcontacts_deleted.initialize(max_value, 100, par.Value.tmin, par.Value.tmax,
					  "ntc_contacts_deleted.out", 
					  "nTCcontacts_deleted");
  ...
  hs.monitor_BsearchTtime_selected.initialize(max_value, 100, par.Value.tmin, par.Value.tmax,
					   "BsearchTtime_selected.out", 
					   "BsearchTtime_selected");
  hs.monitor_BsearchTtime_deleted.initialize(max_value, 100, par.Value.tmin, par.Value.tmax,
  ...
  hs.monitor_BinteractTtime.initialize(max_value, 100, par.Value.tmin, par.Value.tmax,
				    "BinteractTtime.out", "BinteractTtime");
  /* Initialise the counters for the times between two T-B-bindings
   */
  max_value = 20.0;
  hs.monitor_TimeBetweenTbindings.initialize(max_value, 400, par.Value.tmin, par.Value.tmax,
  ...
  hs.monitor_TfhSignalAtSelection.initialize(max_value, 100, par.Value.tmin, par.Value.tmax,
  ...
  hs.monitor_TfhSignalSpeed_selected.initialize(max_value, 100, par.Value.tmin, par.Value.tmax,
					     "TfhSignalSpeed_selected.out", 
					     "TfhSignalSpeed_selected");
  hs.monitor_TfhSignalSpeed_deleted.initialize(max_value, 100, par.Value.tmin, par.Value.tmax,
  ...
  hs.monitor_TFHsignalAUC.initialize(max_value, 100, par.Value.tmin, par.Value.tmax,
   ...
   if (outputfiles == 0) {
     ofstream apolog;
     apolog.open(hyphasma2::prefix_files("apolog.out"));
     apolog << "! time   state   nFDCcontacts  tc_clock  tc_signal   "
	    << "best_affinity  FDCselected_clock\n";
     apolog.close();
   }
}
void cellCC::trackfate_initialize(hstat &hs) {
  fatetracker.mk_new_fate_track(hs);
}
void cellCC::trackfate_show(hstat &hs) {
  fatetracker.show_track(hs);
}
void cellCC::trackfate(double time, bool forceit) {
  if ((fatetracker_n == fatetracker_ndt && state < selected) || forceit) {
    ...
    fatetracker.write_fate(short(state), pos_ss, affinity, time, 
			   ...
			   pMHC_dependent_number_of_divisions);
    fatetracker_n = 0;
  } 
  ++fatetracker_n;
}
void cellCC::writetrackfate(hstat &hs) {
  if (write_trackfate > 0) { fatetracker.write2file(hs); }
  fatetracker.clear_fate_track(hs);
}
// ==============================================================================
...
// ============================================================================
void cellCC::set_statics(const double &time, const Parameter &par, hstat &hs) {
   if (SMOOTH_DIFFERENTIATION) {
      set_differentiation(time, hs);
   } else if (time >= par.Value.StartOutput - 1.0e-09) {
      hs.cc_obj.p_dif2out = p_dif2out_target;
      hs.cc_obj.p_dif2out_DEC = p_dif2out_DEC_target;
      cout << "Output-Production (t=" << time << " hr) (discrete).";
   } else {
      hs.cc_obj.p_dif2out = 0.;
      hs.cc_obj.p_dif2out_DEC = 0.;
   }
}
void cellCC::set_differentiation(const double &time, hstat &hs) {
   /* This routine has to be called at the beginning of a time step only once
    * because it sets a static variable. Note that the starting time
   ...
   if (SMOOTH_DIFFERENTIATION) {
      ...
      hs.cc_obj.p_dif2out = p_dif2out_target / weight;
      hs.cc_obj.p_dif2out_DEC = p_dif2out_DEC_target / weight;
   }
}
...
double cellCC::get_pMHC_presentation() {
   ...
   // ++++++++++++++ OPTION pMHCdeficient ++++++++++++++++++++++++++++++++++++++++++
   // This line is used to mimick pMHC-deficient B cells
   //if (DEC205 && pMHCdeficient) { pMHCp = 0; }
   // Replace this by proper Cre-BCs later ##### 
   // ++++++++++++++ OPTION ++++++++++++++++++++++++++++++++++++++++++++++++++++++++
   if (DEC205_ova) {
     pMHCp = double(pMHC_of_DEC205_ova + nFDCcontacts);
   }
   return pMHCp;
}
...
double cellCC::get_tc_interaction_time() {
  ...
  if (mode_of_setting_tc_time == 1) { // gaussian interaction time
    ...
    pMHClevel = pow(pMHClevel, BTtime_n);
    tt = BTtime_min + 
      (BTtime_max - BTtime_min) * pMHClevel / (pMHClevel + pow(BTtime_K, BTtime_n)); 
  }
  //cout<<"tt="<<tt<<"; ";
  return tt;
}
...
double cellCC::get_ICOSL_expression() {
   ...
   if (ICOSL_upregulation_mode == 1) {
	 ...
	 return 1.0;
      ...
      double tterm = pow(FDCselected_clock,hillcoefficient);
      double icosligand = tterm / (tterm + pow(ICOSL_upregulation_time,hillcoefficient));
      return icosligand;
   }
...
void cellCC::progress_selection_state(AffinitySpace &shape, hstat &hs) {
  /* This is only called in the state unselected and is associated
   * with single or repeated interactions of CC with FDCs.
   * Called from cellCC::select(...) which is called in state contact(to FDC).
   */
  if (TC_CC_selection == 0) {
    ...
    antigen_collection_statistics(nFDCcontacts, true, hs);
...
short cellCC::select(AffinitySpace &shape, hstat &hs) {
   // Assume that cell==CC, is called in the state contact to FDC in cellman::calc_CC(...)
   short int err = 1;
   // Probability of selection:
   if (drandom() < p_sel) {
      // was the FDC-signalling sufficient
      if (drandom() < p_FDCsignalling) {
         ...
         if (collectFDCsignals) {
            // if one arrives here, an additional Ag was collected from FDC; count it
            ++nFDCcontacts;
	    // return to state un(FDC)selected to continue Ag-collection(Tfh-search)
        return2unselected(shape, hs);
         } else {
            progress_selection_state(shape, hs);
         }
      } else {
...
// ============================================================
void cellCC::antigen_collection_statistics(int Ncontacts, bool count_selected, hstat &hs) {
  // save for ag_portions-statistic:
  int agportionstmp = Ncontacts;
  if (agportionstmp > hstat::max_n_of_ag_portions) { agportionstmp = hstat::max_n_of_ag_portions; }
  hs.cc_obj.cummulative_ag_collection_all[agportionstmp]++;
  if (count_selected) {
    hs.cc_obj.cummulative_ag_collection_selected[agportionstmp]++;
  }
}

// ============================================================

short cellCC::stop_collecting_FDCsignals(AffinitySpace &shape, const double& time, double &dt, hstat &hs) {
  ...
  if (collectFDCsignals) {
    ...
    if ( (AgThreshold4Diff > 0 && nFDCcontacts >= AgThreshold4Diff) ||
	 (selected_clock > collectFDCperiod) ) {
      // Monitor this event


      hs.monitor_pureFDCsearch.add(selected_clock);

      // Either kill or select:
      if (nFDCcontacts == 0 && 
	  (inhibitFDCapoptosis == false || drandom()<prob2kill_noFDCcontactBCs) 
	  ) {
	// Case of no FDC contact up to end of collection period
    make_apoptosis(time, shape, hs);
      } else {
	// Case of at least one contact to FDCs
    progress_selection_state(shape, hs);
	shape.rem_cell(sCCunselected,pos_ss); 
	 ...
	 *      is called as well. Need a deeper thinking to do it right.
	 */
...
bool cellCC::same_TC_as_before(long new_tc_index) {
  if (last_tc_index == new_tc_index) return true;
  return false;
}
bool cellCC::try2findTFH() {
  /* Determines whether it is allowed to bind to TFH:
   * Returns TRUE unless, depending ot tc_search_duration_mode,
  ...
  if (state == FDCselected) { 
    ...
    if ( (tc_search_duration_mode == 1 || tc_search_duration_mode == 2) &&
	 FDCselected_clock > tc_search_duration ) 
      { 
	/* For fixed of pMHC-dependent tc_search_duration, the tc_search_duration
	 ...
	 * FoxO is alse active as upper limit of tc_search_duration.
...
double cellCC::get_FoxO_rate(double& pMHClevel) {
  double foxrate = FoxO_production; // works for FoxO_mode==3
  // Put the formula here in dependence on the modes 1 or 2 
  if (FoxO_mode == 1) { // reduce FoxO growth rate with higher pMHC
    double foxi = pow(pMHClevel, FoxOup_n);
    //foxrate = FoxOup_min + 
    //(FoxOup_max - FoxOup_min) * (1.0 - foxi / ( foxi + pow(FoxOup_K, FoxOup_n) ) );
    // also possible:
    foxrate = FoxOup_max - (FoxOup_max - FoxOup_min) * (foxi/(foxi + pow(FoxOup_K, FoxOup_n)));
  } else if (FoxO_mode == 2) { // increase FoxO growth rate with higher pMHC
    double foxi = pow(pMHClevel, FoxOup_n);
    foxrate = FoxOup_min + 
      (FoxOup_max - FoxOup_min) * foxi / ( foxi + pow(FoxOup_K, FoxOup_n) );
  }
  return foxrate;
}
void cellCC::bind_TC(const double& time, cellTC &tcell, space &l, AffinitySpace &shape, hstat &hs) {
  ...
  if (TimeOfLastTbinding > 0) // meaning there was a contact before
    { hs.monitor_TimeBetweenTbindings.add(time - TimeOfLastTbinding); }
  // cout<<"bind2TC!\n";
...
void cellCC::make_apoptosis(const double& time, AffinitySpace &shape, hstat &hs) {
  //   if (outputfiles == 0) {
  ofstream apolog;
  apolog.open(hyphasma2::prefix_files("apolog.out"), ofstream::app);
  apolog << time << "  " 
	 << state << "  "
	 << nFDCcontacts << "  " 
	 << tc_clock << "  "
  ...
  antigen_collection_statistics(nFDCcontacts, false, hs);
  hs.monitor_nTCcontacts_deleted.add(nTCcontacts);
  hs.monitor_BsearchTtime_deleted.add(FDCselected_clock);
  hs.monitor_TfhSignalAtSelection.add(tc_signal);
  hs.monitor_TfhSignalSpeed_deleted.add(calc_TfhSignalSpeed());
  hs.monitor_TFHsignalAUC.add(TFHsignalAUC);
...
double cellCC::get_pMHC_dependent_division() {
    ...
    * antigens are used.
    */
   double ag_factor = pow(pMHC,pMHC_dependent_nHill);
   double ndiv = pMHC_dependent_P_min
                 + (pMHC_dependent_P_max - pMHC_dependent_P_min)
                 * ag_factor / (ag_factor + pow(pMHC_dependent_K,pMHC_dependent_nHill));
   // +++++++++++++++ OPTION pMHCdeficient ++++++++++++++++++++++++++++++++++++++++++++++
   //if (DEC205 && pMHCdeficient) { ndiv = 0; }
   //if (DEC205) { cerr << "GOTIT=" << ndiv << "; "; }
   // +++++++++++++++ OPTION pMHCdeficient ++++++++++++++++++++++++++++++++++++++++++++++
   return ndiv;
}
// ============================================================
...
double cellCC::get_signal_induced_number_of_divisions() {
  ...
  if (mode_of_DND_of_Tfh == 0) { // i.e. DND is calculated from absolute signal levels
    double ag_factor = pow(tc_signal, pMHC_dependent_nHill);
    ndiv = pMHC_dependent_P_min
      + (pMHC_dependent_P_max - pMHC_dependent_P_min)
      * ag_factor / (ag_factor + pow(TFHsignal_dependent_K,pMHC_dependent_nHill));
  } else if (mode_of_DND_of_Tfh == 2) { // use AUC
    double ag_factor = pow(TFHsignalAUC, pMHC_dependent_nHill);
    ndiv = pMHC_dependent_P_min
      + (pMHC_dependent_P_max - pMHC_dependent_P_min)
      * ag_factor / (ag_factor + pow(TFHsignal_dependent_K,pMHC_dependent_nHill));
  } else { // mode_of_DND_of_Tfh == 1, i.e. DND is calculated from the gradient of signal
    double sig_speed = tc_signal / FDCselected_clock;
    double sig_term = pow(sig_speed, pMHC_dependent_nHill);
    ndiv = pMHC_dependent_P_min
      + (pMHC_dependent_P_max - pMHC_dependent_P_min)
      * sig_term / (sig_term + pow(TFHgradient_dependent_K, pMHC_dependent_nHill));
  }
  return ndiv;
}
// ============================================================

short cellCC::get_tc_selected(AffinitySpace &shape, hstat &hs) {
  ...
  if ( (AgThreshold4Selection > 0 && nFDCcontacts > AgThreshold4Selection) ||
    ...
    if (pMHC_dependent_division) {
      ...
      pMHC_dependent_number_of_divisions = get_signal_induced_number_of_divisions();
    }
    //cerr<<", NDIV="<<pMHC_dependent_number_of_divisions<<"\n";
    shape.add_cell(sCCselected,pos_ss);
    if (state == TCcontact) {
    ...
    // For the statistics:
    antigen_collection_statistics(nFDCcontacts, true, hs);
    hs.monitor_nTCcontacts_selected.add(nTCcontacts);
    hs.monitor_BsearchTtime_selected.add(FDCselected_clock);
    hs.monitor_TfhSignalAtSelection.add(tc_signal);
    hs.monitor_TfhSignalSpeed_selected.add(calc_TfhSignalSpeed());
    hs.monitor_TFHsignalAUC.add(TFHsignalAUC);
    // change states ...
    state = selected;
    selected_clock = 0.;
...
bool cellCC::do_selection() {
  /* Called by try_selection(...) only (see next routine).
...
short cellCC::try_selection(const double &time, const double &dt, AffinitySpace &shape, hstat &hs) {
  ...
  if (do_selection()) {
      ...
      */
     short gotit = get_tc_selected(shape, hs);
     ...
     if ( ((gotit == 1) && (TCselect_prob < 1) && (drandom() > TCselect_prob)) ) { 
       ...
       hs.monitor_nTCcontacts_selected.rm(nTCcontacts);
       hs.monitor_BsearchTtime_selected.rm(FDCselected_clock);
       hs.monitor_TfhSignalAtSelection.rm(tc_signal);
       hs.monitor_TfhSignalSpeed_selected.rm(calc_TfhSignalSpeed());
       hs.monitor_TFHsignalAUC.rm(TFHsignalAUC);
     }
...
double cellCC::get_TFHsignal_delivery_factor(double pMHC, bool calculate) {
  ...
  double ag_factor = pow(pMHC, TFHsignal_delivery_n);
  double factor = TFHsignal_delivery_min
    + (TFHsignal_delivery_max - TFHsignal_delivery_min)
    * ag_factor / (ag_factor + pow(TFHsignal_delivery_KpMHC, TFHsignal_delivery_n));
  return factor;
}
void cellCC::add_tc_signal() {
  ...
  */
  ...
  if (TFHsignal_delivery_mode == 1) {
    portion *= get_TFHsignal_delivery_factor( get_pMHC_presentation(), false );
  }
  tc_signal += portion;
...
short cellCC::got_tc_signals(const double &time, const double &dt, cellTC &tcell, 
   ...
   if (try2select) {
      ...
      * is taken back in the state FDCselcalc_ccected in try_selection(...).
      */
     if (tc_search_duration_mode == 0 || force_selection_at_TFHthreshold) {
       gotit = get_tc_selected(shape, hs);
       /* The parameter TCell (=TCselect_prob) in setparam controls if all successful BC are
	* surviving (default=1) */
       if ((gotit == 1) && (TCselect_prob < 1) && (drandom() > TCselect_prob)) {
      ...
      */
     if (gotit == 0 
	 && ( multipleTFHcontacts || tc_search_duration_mode > 0) // works for 1..4
     ...
     if (gotit == 1) { 
       hs.monitor_BinteractTtime.add(tc_clock);
...
short cellCC::dif2OUTorCB(double &dt, hstat &hs) {
  ...
  if (DEC205_ova) {
    ...
    if (drandom() < hs.cc_obj.p_dif2out_DEC) {
    ...
    if (drandom() < hs.cc_obj.p_dif2out) { // p_dif2out is a probability not a rate!
	  selected4output = true;
	  if (p_final_differentiation > 0) { return 3; } 
	  else { return 1; }
	} else { return 2; }
      }
    }
...
// ============================================================
short cellCC::apoptose(const double &time, AffinitySpace &shape, hstat &hs) {
   // cout<<"\napoptose";
   short int err = 1;
    ...
    * or when in tc_search_duration_mode==3 FoxO>1 is reached,
    * or when in tc_search_duration_mode==4 and FoxO>1 and tc_signal never reached SST_tc_signal,
    * 
    */
...
short cellCC::macrophagocyte(AffinitySpace &shape, hstat &hs) {
   if (drandom() < p_mph) {
      /// Philippe : don't get it : n_FDC_encounters is the number of failed interactions ???
      if (n_fdc_encounters < hstat::fdc_max_encounters) {
         ++hs.cc_obj.fdc_encounters[n_fdc_encounters];
      } else { ++hs.cc_obj.fdc_encounters[hstat::fdc_max_encounters - 1]; }
...
// ============================================================
void cellCC::update_histograms(hstat &hs) {
  hs.monitor_pureFDCsearch.update_day();
  hs.monitor_nTCcontacts_selected.update_day();
  hs.monitor_nTCcontacts_deleted.update_day();
  hs.monitor_BsearchTtime_selected.update_day();
  hs.monitor_BsearchTtime_deleted.update_day();
  hs.monitor_BinteractTtime.update_day();
  hs.monitor_TimeBetweenTbindings.update_day();
  hs.monitor_TfhSignalAtSelection.update_day();
  hs.monitor_TfhSignalSpeed_selected.update_day();
  hs.monitor_TfhSignalSpeed_deleted.update_day();
  hs.monitor_TFHsignalAUC.update_day();
}
void cellCC::write_histograms(hstat &hs) {
  hs.monitor_pureFDCsearch.write2file();
  hs.monitor_nTCcontacts_selected.write2file();
  hs.monitor_nTCcontacts_deleted.write2file();
  hs.monitor_BsearchTtime_selected.write2file();
  hs.monitor_BsearchTtime_deleted.write2file();
  hs.monitor_BinteractTtime.write2file();
  hs.monitor_TimeBetweenTbindings.write2file();
  hs.monitor_TfhSignalAtSelection.write2file();
  hs.monitor_TfhSignalSpeed_selected.write2file();
  hs.monitor_TfhSignalSpeed_deleted.write2file();
  hs.monitor_TFHsignalAUC.write2file();
}
void cellCC::show_cummulative_ag_collection(hstat &hs) {
   ofstream aghisto(hyphasma2::prefix_files("ag_endhisto.out"));
   ...
   for (int i = 0; i <= hstat::max_n_of_ag_portions; i++) {
      aghisto << i
              << "   " << hs.cc_obj.cummulative_ag_collection_selected[i]
              << "   " << hs.cc_obj.cummulative_ag_collection_all[i]
              << "\n";
   }
   aghisto.close();
}
...
cellTC::cellTC(const cellTC &x) : cell(x) {
   /// Philippe danger 
   /// MMH: could you please explain your problem. It is fine to copy frag_cell first
   /// and when you put this comment here, it was not even a frag_cell.
...
void cellFDC::set_statics(const Parameter &par, space &l, ofstream &ana, hstat &hs) {
   if (par.Value.mksignal <= 0.) {
      hs.fdc_obj.p_mksignal = 0.;
   } else { hs.fdc_obj.p_mksignal = par.Value.mksignal * par.Value.deltat; }
   ana << "FDC differentiation signal production = " << hs.fdc_obj.p_mksignal << "\n";
...
void cellFDC::set_statics(const double &time, const Parameter &par, hstat &hs) {
   if (time >= par.Value.Start_Differentiation - 1.0e-09) {
      if (par.Value.mksignal < 1.e-08) { hs.fdc_obj.p_mksignal = 0.; } else {
         hs.fdc_obj.p_mksignal = par.Value.mksignal * par.Value.deltat;
         cout << "Signal production, ";
      }
   } else { hs.fdc_obj.p_mksignal = 0.; }
   // CXCL13 and SEMA4D are produced all the time! Thus only the first set_statics is used.
}
// ============================================================

void cellFDC::signal_production(const long &i, sigs &l, hstat &hs) {
   // Produce signal for differentiation of CB to CC and for chemotaxis
   if (l.signal_use[sig_differ2CC] == 1) { signal_secretion(i,sig_differ2CC,vesicle,hs.fdc_obj.p_mksignal,l); }
...
double cellFDC::get_total_immune_complex() {
   ...
   * This version is used if no affinity bins are used 
   * (with affinity bins see overloaded function below)
   */
   double t = 0.;
   for (int f = 0; f < volume; f++) {
      for (int a = 0; a < n_Antigen_max; a++) {
         // see comment in get_total_antigen()
         t += ic_amount[f][0][a];    // everything is in the first bin-element
      }
   }
   return t;
}
double cellFDC::get_total_immune_complex(const int &length) {
  /* This function is used exclusively for read-out (v2016-02-06).
   * This version is used with affinity bins 
   * (without affinity bins see overloaded function above)
   */
   double t = 0.;
   for (int f = 0; f < volume; f++) {
...
double cellFDC::get_total_immune_complex(const int &ag_index, const int &length) {
   // this function is used exclusively for read-out (v2016-02-06)
   // this version is used with affinity bins (without affinity bins see overloaded function above)
...
short cellFDC::consume_ag(const long &frag_index, const int &ag_index) {
  ...
  if ((antigen_amount[frag_index][ag_index] >= 1.)
    ...
    --antigen_amount[frag_index][ag_index];
    //--antigen_amount[frag_index][ag_index]; //<GClifetime paper> uncomment for simulations with 2 ag consumption
...
void cellFDC::mk_immune_complex(const double &d_t, AntibodyDyn &ABS, AffinitySpace &AS, hstat &hs) {
   ...
   for (int f = 0; f < volume; f++) {
      ...
      for (int a = 0; a < n_Antigen; a++) {
         ...
         for (int i = 0; i <= ABS.antibodies_resolution; i++) {
            ...
            //double ab = 1.e+15 * ABS.ab_bins[a].antibodies[i]; //commented this
            double ab = 1.e+15 * hyphasma2::ab_sum[i][a];     // antibodies[i] in
            ...
            if ((ic_amount[f][i][a] < 0.)
               ...
               if ((ic_amount[f][i][a] < -1.e-15)
                  ...
                  cout << "frag " << f << ": kon=" << ABS.ab_bins[a].k_on[i] << "; koff="
                       ...
                       << "; ab=" << hyphasma2::ab_sum[i][a]
                       << "; ag=" << antigen_amount[f][a]
                       << ": konterm=" << d_t * ABS.ab_bins[a].k_on[i]
                  * antigen_amount[f][a] * hyphasma2::ab_sum[i][a]
                       << "; koffterm=" << d_t * ABS.ab_bins[a].k_off[i] * ic_amount[f][i][a]
                       << "; ic=" << ic_amount[f][i][a] << "; d_ic=" << d_ic[i] << "\n";
                  // exit(1);
               }
               if (ic_amount[f][i][a] < 0.) { ic_amount[f][i][a] = 0.; }
               if (hyphasma2::ab_sum[i][a] < 0.) { hyphasma2::ab_sum[i][a] = 0.; }
...
void cellFDC::mk_immune_complex(const double &d_t, sigs &l, hstat &hs) {
...
//double cellOUT::average_affinity[MAX_GCS] ={ 0.};
//double cellOUT::max_affinity[MAX_GCS] ={ 0. };
...
void cellOUT::set_statics(const Parameter &par, space &l, ofstream &ana, hstat &hs) {
   ...
   if (par.Value.mk_ab <= 0.) {
      cellOUT::p_mk_ab = 0.;
   } else { cellOUT::p_mk_ab = par.Value.mk_ab * par.Value.deltat / par.Value.ag_threshold; }

   vesicle = par.Value.FDCvesicle;
   ana << "OUT antibody production mode (=FDC-mode) = " << vesicle << "\n";

   cellOUT::initial_ab_affinity = par.Value.initial_ab_affinity;
   if (cellOUT::initial_ab_affinity < 0) {
      hs.out_obj.average_affinity = hs.cb_obj.average_seeder_affinity;
      hs.out_obj.max_affinity = hs.cb_obj.average_seeder_affinity;
   } else {
      hs.out_obj.average_affinity = cellOUT::initial_ab_affinity;
      hs.out_obj.max_affinity = cellOUT::initial_ab_affinity;
   }
    ...
    */
   cellOUT::use_threshold= par.Value.use_ab_dynamics;
   ana << "  Ag presenting Abs adapt to PC secreted Ab-affinity = " << cellOUT::use_threshold << "\n";
}
void cellOUT::signal_production(const long &i, sigs &l) {
   // Produce antibodies
   signal_secretion(i,antibody,vesicle,cellOUT::p_mk_ab,l);
}
short cellOUT::try_eject(long i, space &l, AffinitySpace &shape) {
   if (l.no_border(i) == 0) {
      // Fuege die Zelle zum output hinzu
      shape.add_cell(soutext,pos_ss);
      // sout zaehlt alle jemals erzeugten output Zellen = keine Aenderung
      // total wird nicht korrigiert, da alle lebenden Zellen gezaehlt werden
      return 0;
   }
   return 1;
}
...
void cellbeta::open_files() {
   ...
   result.open(hyphasma2::prefix_files(filename));
   ...
   strcat(filename,beta_index);
   strcat(filename,".out");
   result2.open(hyphasma2::prefix_files(filename));
   result2 << "! t[s] g_sK_Ca gbar_gap g_fNa_V\n";

   strcpy(filename,"beta_i");
   strcat(filename,beta_index);
   strcat(filename,".out");
   currents.open(hyphasma2::prefix_files(filename));
   ...
   strcat(filename,beta_index);
   strcat(filename,".out");
   currents_rho.open(hyphasma2::prefix_files(filename));
   ...
   strcat(filename,beta_index);
   strcat(filename,".out");
   reversal.open(hyphasma2::prefix_files(filename));
   reversal << "! t[s] V[mV]  Vbar_K  Vbar_Na  Vbar_Ca  Vbar_Ca_ER\n";

   strcpy(filename,"beta_g");
   strcat(filename,beta_index);
   strcat(filename,".out");
   gapfile.open(hyphasma2::prefix_files(filename));
...
double cellbeta::get_Hill(double &x, double &half, double &coefficient) {
   return pow(x,coefficient) / (pow(x,coefficient) + pow(half,coefficient));
}
...
double cellbeta::move(const long &li, space &l, sigs &s, TRACK &td, double &time, hstat &hs) {
   // call polarisation
   set_polarity_velocity(persistence,v_modi,p_switch_v,n_v_states,v_slow_factor,l,s);
   if (writethis2track == polarisation) {
