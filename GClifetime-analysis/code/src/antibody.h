...
class AntibodyBin {
  public:
   ...
   void write_ab_tc(double &time);
   void make_self_ic(Parameter &par);// Immune complex of antibodies with test antigen for IP calc
   void clear_self_ics();// clear immune complexes

   double get_total_self_ic();
   double get_total_self_ic_pergc();
   double get_total_self_ab();
   double get_total_self_ab_pergc();

   void update_ab_new(vector<double> ab_ext, long int ntcount);
   double * ip_self;
   double * ip_self_pergc;
   double * self_ab; // Abs produced by simulated GCs
   double * self_ab_pergc; // Abs produced by simulated GCs /N
   static double gfree;// For IP calc
   static double gfree_pergc;// IP calc/N
 ...
 * is
 *  created in the Affinity Space, a new AntibodyBin is created for it.
 */
class AntibodyDyn {
  public:
   // Konstruktor
   ...
   /** @brief Main storage : ab_bins[i] = AntibodyBin for the antigen nr i in the Affinity Space */
   ...
   void produce_antibodies_outside(AffinitySpace &AS, hstat &hs);
   ...
   void update_antibody_delay(double use_delay, double time);

   ///Philippe : add this functions and put the parameters in static
   static void set_statics(const Parameter &par, ofstream &ana, hstat &hs);
   ...
   //static double antibody_production_factor[1000];
