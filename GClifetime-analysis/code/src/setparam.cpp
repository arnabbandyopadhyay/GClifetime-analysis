...
void Werte::inialld() {
   ...
   pm_apoptosis_time=24.0; // in hours, this parameter is not yet included in the parfile
...
const char* Werte::kenntext(int nummer) {
   ...
   */
   ...
   case 395: { // THFgradient_of_P0divisions
     return 
...
ofstream&Werte::fPut(ofstream &s) {
   ...
   s << "----------------------------------------------------------------------\n";
   s << "Set Possible modes of BC search for Tfh help and apoptosis = 3.\n"
   ...
   s << "Either pMHC- or Tfh-signal-dependent can be used. If Tfh-signal is used,\n"
     << "parameters below are interpreted as signal in hours.\n"
     << "---\n"
   ...
   s << "From here specific for pMHC-dependent number of division:\n"
     << "Still used in Tfh-signal-dependent DND to determine pMHC-level in DEC205-OVA-exp.\n";
   s << kenntext(289) << ":\n" << pMHC_dependent_K << "\n";
...
void Werte::fGet(char * parname, bool transform2rate) {
   ...
   */
   ...
   if (fFind(parname, s, 394) == 1) {
      s >> TFHgradient_dependent_K;
   }
   if (fFind(parname, s, 395) == 1) {
      s >> TFHgradient_of_P0divisions;
   }
