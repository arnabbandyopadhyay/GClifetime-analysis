...
void TRACK::Show_tracks() {
   ...
   ofstream ff2(hyphasma2::prefix_files("trackraw.out"));
   ff2 << N_OBJECTS << "  "
       << TRACKFROM << "  "
       << TRACKUNTIL << "  "
       << DELTA_T << "  "
...
void TRACK::make_trace_types_a() {
   ...
   for (short c = CB; c < N_cells; c++) {
      ...
      if (nc > 0) {
         ...
         strcat(datname,".gle");
         ofstream x(hyphasma2::prefix_files(datname));
         char cellname[10] = "";
...
void TRACK::make_traces_r() {
   ofstream x(hyphasma2::prefix_files("traces_r.gle"));
   x << "size 15 15\n" << "amove 2 2\n" << "set font TEXCMR\n" << "begin graph\n"
     << "  size 12 12\n" << "  fullsize\n"
     << "  xtitle \" ";
   if (dim == 2) { x << "x"; } else if (dim == 3) {
...
void TRACK::make_traces_a() {
   ofstream x(hyphasma2::prefix_files("traces_a.gle"));
   x << "size 15 15\n" << "amove 2 2\n" << "set font TEXCMR\n" << "begin graph\n"
     << "  size 12 12\n" << "  fullsize\n"
     << "  xtitle \" ";
   if (dim == 2) { x << "x"; } else if (dim == 3) {
...
void TRACK::make_reach_dist_x() {
   ofstream x(hyphasma2::prefix_files("reach_dist_x.gle"));
   x << "size 15 15\n" << "amove 2 2\n" << "set font TEXCMR\n" << "begin graph\n"
     << "  size 12 12\n" << "  fullsize\n"
...
void TRACK::Write_trace_begin(char prename[20]) {
   ...
   for (short c = CB; c < N_cells; c++) {
      ...
      if (nc > 0) {
         ...
         strcat(datname,typeno);
         strcat(datname,".out");
         ofstream fff(hyphasma2::prefix_files(datname));
...
void TRACK::Write_trans_zone(char prename[20], double * hess, double thick) {
   ...
   // Open the file
   ofstream fff(hyphasma2::prefix_files(strcat(prename,".out")));
...
void TRACK::Write_reached_distance() {
    ...
    */
   ofstream fff(hyphasma2::prefix_files("reach_dist.out"));
   ofstream ffx(hyphasma2::prefix_files("reach_dist_x.out"));
...
void TRACK::Write_turning_angle() {
   ...
   ofstream fff;
   fff.open(hyphasma2::prefix_files("turning_angle.out"));
...
void TRACK::Write_speed() {
   ...
   ofstream fff(hyphasma2::prefix_files("v_t.out"));
   ...
   ofstream ffx(hyphasma2::prefix_files("vmean_t.out"));
...
void TRACK::Write_fdc_contact_times() {
   ...
   ofstream fff;
   fff.open(hyphasma2::prefix_files("fdccontact.out"));
...
void TRACK::Write_shape() {
   ...
   ofstream fff(hyphasma2::prefix_files("axis_t.out"));
   ...
   ofstream ffx(hyphasma2::prefix_files("axismean_t.out"));
