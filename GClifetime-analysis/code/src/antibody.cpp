...
double AntibodyDyn::antibody_degradation = 30.0;            // Antibody degradation half time
                                                            // (days):
//double AntibodyDyn::antibody_production_factor[1000] = {1e-17};     // Production rate of systemic
                                                            // antibodies by output population (mol
                                                            // per hour and cell):
...
double AntibodyBin::gfree=(1.0000e-6);
                                                            // AntibodyDyn::check4consistency(...)
double AntibodyBin::gfree_pergc=(1.0000e-6);

//double AntibodyBin::gfree=3e-5;
//double AntibodyBin::gfree_pergc=3e-5;
...
void AntibodyBin::initialise(int &resolution, int &ag,
   ...
   ip_self=new double[bindim+2];
   ip_self_pergc=new double[bindim+2];
   self_ab=new double[bindim+2];
   self_ab_pergc=new double[bindim+2];
   ...
   if (inject_ab_ASindex < 0) {
      ...
      double injection_affinity = AS.affinity(inject_ab_ASindex,ag_index);
   ...
   if (ag >= 0) {
      ...
      ofstream ab_file(hyphasma2::prefix_files(datname));
...
void AntibodyBin::write_antibodies(double &time) {
   ...
   get_filename(datname);
   ofstream ab_file;
   ab_file.open(hyphasma2::prefix_files(datname), ofstream::app);
   // add things
   double total_ab = get_total_antibodies();
   ab_file << time/24 << "   " << total_ab << " ";   // in M
...
void AntibodyBin::update_ab_new(vector<double> ab_ext, long int ntcount)
{
    for (int i=0; i<11; i++){
        antibodies[i] +=ab_ext[ntcount];
        ntcount++;
    }
}

void AntibodyBin::write_ab_tc(double &time) {
    double antibody_conc;
    ofstream ab_tc;

    ab_tc.open(hyphasma2::prefix_files("ab_tc1.out"), ofstream::app);
    //ab_tc << "Time_" << time << ":" << "\n";
    ab_tc << "\n";

    for(int i=0;i<=bindim;i++){
        antibody_conc=antibodies[i];
        ab_tc << " " << antibody_conc << "\n";
    }
    ab_tc << "\n";
    ab_tc.close();
...
void AntibodyDyn::set_statics(const Parameter &par, ofstream &ana, hstat &hs) {
   ...
   hs.antibody_production_factor = 0;
   if (par.Value.antibodies_production > 0) {
      hs.antibody_production_factor = par.Value.antibodies_production * par.Value.deltat
                                   * par.Value.N_GC
                                   / (par.Value.V_blood * 1.e+15);
   }
   // unit mol/micrometer^3 per cell and hour (note for V_blood in liter: 1.e+13 micrometer^3, i.e.
   // 10^-2 liter)
   ana << "  Effective Ab production factor = " << hs.antibody_production_factor
       << " mol/micrometer^3\n";
...
void AntibodyDyn::produce_antibodies_outside(AffinitySpace &AS, hstat &hs) {
 ...
 */
   ...
   for (int n = 0; n < n_producers; n++) {
      ...
      d_ab = AS.get_AbProducingCells(n) * hs.antibody_production_factor
...
void AntibodyDyn::update_antibody_delay(double delay, double time)
{
    ...
    vector<double> st = vector<double>(11,100);


    string fileToOpen;
    if(delay<100) fileToOpen = string("ab_tc1_p.out");
    if(delay>=100 && delay<200) fileToOpen = string("ab_tc2_p.out");
    if(delay>=200 && delay<300) fileToOpen = string("ab_tc3_p.out");
    if(delay>=300 && delay<400) fileToOpen = string("ab_tc4_p.out");
    if(delay>=400 && delay<500) fileToOpen = string("ab_tc5_p.out");
    if(delay>=500) fileToOpen = string("ab_tc6_p.out");
    if(fileToOpen.size() == 0) cerr << "ERR: No file to open" << endl;

    std::ifstream ac = ifstream(fileToOpen.c_str()); // ifstream takes a char[], so need to convert the string by c_str()
    if(!ac){
        cerr << "ERR: couldn't open " << fileToOpen << endl;
        return;
    }

    bool found=false;
    int cpt = 0;
    while((!found) && (!ac.eof()) && (cpt < 100000))
    {
        string header;
        ac >> header;

        if(!header.compare(k)){ // compare returns 0 when words are equal
            found = true;
            double val;
            //cout << "Found delayed feedback for " << k << " with values ";
            for(int j = 0; j < 11; ++j){
                ac >> val;
                //cout << "\t" << val;
                ab_bins[0].antibodies[j]=ab_bins[0].antibodies[j]+val;
            }
            //cout << endl;
        } else { // discards the next 11 lines
            double val;
            for(int j = 0; j < 11; ++j){
                ac >> val; //don't do anything with the value
            }
        }
        cpt++;
    }
...
void AntibodyBin::clear_self_ics(){
...
void AntibodyBin::make_self_ic(Parameter &par){

    for(int i=0;i<AntibodyDyn::antibodies_resolution+1;i++){
        double ki=0;
        ki=pow(10,-5.5-0.4*i);

        double ip_self_temp=0.0;
        double ip_self_temp_pergc=0;

        self_ab[i]=antibodies[i]*1.e+15;
        self_ab_pergc[i]= (antibodies[i]/par.Value.N_GC)*1.e+15;

        ip_self_temp=(antibodies[i]*1.e+15*gfree)/(ki+gfree);
        if(ip_self_temp>antibodies[i]*1.e+15)
        {
            ip_self_temp=antibodies[i]*1.e+15;
        }

        if(ip_self_temp>gfree)
        {
            ip_self_temp=gfree;
        }
        ...
        ip_self_temp_pergc=((antibodies[i]/par.Value.N_GC)*1.e+15*gfree_pergc)/(ki+gfree_pergc);

        if(ip_self_temp_pergc>(antibodies[i]/par.Value.N_GC)*1.e+15)
        {
            ip_self_temp_pergc=(antibodies[i]/par.Value.N_GC)*1.e+15;
        }

        if(ip_self_temp_pergc>gfree_pergc)
        {
            ip_self_temp_pergc=gfree_pergc;
        }
        if(ip_self_temp_pergc<0){
            ip_self_temp_pergc=0;
        }

        ip_self_pergc[i]+=ip_self_temp_pergc;
        ip_self[i]+=ip_self_temp;
...
double AntibodyBin::get_total_self_ic(){
    double ipself_tt=0.0;
    for(int i=0;i<AntibodyDyn::antibodies_resolution+1;i++){
        if(ip_self[i]>0)
        {
        ipself_tt+=ip_self[i];
...
double AntibodyBin::get_total_self_ic_pergc(){
    double ipself_tt_pergc=0.0;
    for(int i=0;i<AntibodyDyn::antibodies_resolution+1;i++){
        if(ip_self_pergc>0)
        {
        ipself_tt_pergc+=ip_self_pergc[i];
        }
    }
    return ipself_tt_pergc;

}
double AntibodyBin::get_total_self_ab(){
    double abself_tt=0.0;
    for(int i=0;i<AntibodyDyn::antibodies_resolution+1;i++){
        abself_tt+=self_ab[i];
    }
    return abself_tt;
...
double AntibodyBin::get_total_self_ab_pergc(){
    double abself_tt_pergc=0.0;
    for(int i=0;i<AntibodyDyn::antibodies_resolution+1;i++){

        abself_tt_pergc+=self_ab_pergc[i];
    }
    return abself_tt_pergc;
